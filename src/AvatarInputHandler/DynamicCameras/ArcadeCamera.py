...
# File: ArcadeCamera.pyc (Python 2.7)
...
class CollisionVolumeGroup(namedtuple('CollisionVolumeGroup', ('minVolume', 'lowSpeedLimit', 'vehicleVisibilityLimit', 'approachSpeed', 'cameraSpeedFactor', 'criticalDistance', 'canSkip'))):
    ...
    def __new__(cls, minVolume = 0, lowSpeedLimit = 0, vehicleVisibilityLimit = 0, approachSpeed = Math.Vector2(1.5, 10000), cameraSpeedFactor = 0.1, criticalDistance = 5, canSkip = False):
    ...
    fromSection = staticmethod(fromSection)
...
class _InputInertia(object):
    ...
    def isGliding(self):
        if self._InputInertia__deltaEasing.value != math_utils.VectorConstant.Vector3Zero:
            pass
        return not (self._InputInertia__deltaEasing.stopped)
...
class ArcadeCamera(CameraWithSettings, CallbackDelayer, TimeDeltaMeter):
    ...
    _FOCAL_POINT_CHANGE_SPEED = 100
    _FOCAL_POINT_MIN_DIFF = 5
    ...
    def _ArcadeCamera__getVehicleMProv(self):
        refinedMProv = self._ArcadeCamera__aimingSystem.vehicleMProv
        return refinedMProv.b.source

    
    def _ArcadeCamera__setVehicleMProv(self, vehicleMProv):
        if self._ArcadeCamera__cam.aimPointProvider is not None:
            pass
        prevAimRel = None
        ...
        self._ArcadeCamera__aimingSystem.update(0)
    ...
    def __init__(self, dataSec, defaultOffset = None):
        ...
        self._ArcadeCamera__focalPointDist = 1
    ...
    _getConfigsKey = staticmethod(_getConfigsKey)
    ...
    def create(self, onChangeControlMode = None, postmortemMode = False, smartPointCalculator = True):
        ...
        if not postmortemMode:
            pass
        self._ArcadeCamera__aimingSystem = aimingSystemClass(self._ArcadeCamera__refineVehicleMProv(targetMat), self._cfg['heightAboveBase'], self._cfg['focusRadius'], self._ArcadeCamera__calcAimMatrix(), self._cfg['angleRange'], smartPointCalculator)
        if self._ArcadeCamera__adCfg['enable']:
    ...
    def _ArcadeCamera__setDynamicCollisions(self, enable):
    ...
    def enable(self, preferredPos = None, closesDist = False, postmortemParams = None, turretYaw = None, gunPitch = None, camTransitionParams = None, initialVehicleMatrix = None, arcadeState = None):
        ...
        if player.observerSeesAll() and player.arena.period == constants.ARENA_PERIOD.BATTLE and vehicle and vehicle.id == player.playerVehicleID:
            self.delayCallback(0, self.enable, preferredPos, closesDist, postmortemParams, turretYaw, gunPitch, camTransitionParams, initialVehicleMatrix)
            return None
        if initialVehicleMatrix is None:
            if vehicle is None:
                pass
            initialVehicleMatrix = vehicle.matrix
        ...
        camDist = None
        if self._ArcadeCamera__postmortemMode or closesDist:
        ...
        self.delayCallback(0, self._ArcadeCamera__cameraUpdate)
        g_guiResetters = g_guiResetters
        import gui
    ...
    def _ArcadeCamera__setupCameraTransition(self, duration):
    ...
    def _ArcadeCamera__refineVehicleMProv(self, vehicleMProv):
    ...
    def _ArcadeCamera__setupCameraProviders(self, vehicleMProv):
    ...
    def _ArcadeCamera__setCameraPosition(self, relativeToVehiclePosition):
        self._ArcadeCamera__cam.cameraPositionProvider.a = Vector4(relativeToVehiclePosition.x, relativeToVehiclePosition.y, relativeToVehiclePosition.z, 1)

    
    def _ArcadeCamera__setCameraAimPoint(self, relativeToVehiclePosition):
        self._ArcadeCamera__cam.aimPointProvider.a = Vector4(relativeToVehiclePosition.x, relativeToVehiclePosition.y, relativeToVehiclePosition.z, 1)
    ...
    def disable(self):
        ...
        BigWorld.setMinLodBiasForTanks(0)
        self._ArcadeCamera__collideAnimatorEasing.stop()
        self._ArcadeCamera__cam.shiftCamera(Vector3(0))
    ...
    def update(self, dx, dy, dz, rotateMode = True, zoomMode = True, updatedByKeyboard = False):
        ...
        if updatedByKeyboard:
            pass
        self._ArcadeCamera__curSense = self._ArcadeCamera__sensitivity
        if updatedByKeyboard:
            pass
        self._ArcadeCamera__curScrollSense = self._ArcadeCamera__scrollSensitivity
    ...
    def _ArcadeCamera__updateAngles(self, dx, dy):
    ...
    def _ArcadeCamera__update(self, dx, dy, dz, rotateMode = True, zoomMode = True):
        if not self._ArcadeCamera__aimingSystem:
            return None
        eScrollDir = None.convertDZ(dz)
        ...
        if (isColliding or collisionWhileGlide) and eScrollDir is EScrollDir.OUT:
            pass
        preventScrollOut = not self._ArcadeCamera__compareCurrStateSettingsKey(GAME.COMMANDER_CAM)
        if preventScrollOut and prevDist == distMinMax.max and self._ArcadeCamera__isSettingsEnabled(GAME.COMMANDER_CAM):
            if self._ArcadeCamera__isInArcadeZoomState() or not self._ArcadeCamera__isSettingsEnabled(GAME.PRE_COMMANDER_CAM) or self._ArcadeCamera__compareCurrStateSettingsKey(GAME.PRE_COMMANDER_CAM):
        ...
        if abs(newDist - prevDist) < floatEps and math_utils.almostZero(newDist - distMinMax.min):
            if self._ArcadeCamera__isInArcadeZoomState() and self._ArcadeCamera__onChangeControlMode and not (self._ArcadeCamera__updatedByKeyboard):
                ...
                return None
            None._ArcadeCamera__changeZoomState(EScrollDir.IN)
    ...
    def _ArcadeCamera__adjustMinDistForShotPointCalc(self):
    ...
    def _ArcadeCamera__startInputInertiaTransition(self, prevPos, duration = _DEFAULT_ZOOM_DURATION, easing = EXPONENTIAL_EASING):
    ...
    def _ArcadeCamera__checkNewDistance(self, newDistance):
        distMinMax = self._ArcadeCamera__distRange
    ...
    def _ArcadeCamera__updateProperties(self, state = None):
        ...
        if state:
            pass
        self._ArcadeCamera__distRange = self._cfg['distRange']
        if state:
            pass
        self._ArcadeCamera__overScrollProtectOnMax = self._cfg['overScrollProtectOnMax']
        if state:
            pass
        self._ArcadeCamera__overScrollProtectOnMin = self._cfg['overScrollProtectOnMin']
        if state:
            pass
        self._ArcadeCamera__sensitivity = self._cfg['sensitivity']
        if state:
            pass
        self._ArcadeCamera__scrollSensitivity = self._cfg['scrollSensitivity']
        if state is None:
            if self._ArcadeCamera__isSettingsEnabled(GAME.PRE_COMMANDER_CAM):
                self._ArcadeCamera__overScrollProtectOnMax = 0
            self._ArcadeCamera__zoomStateSwitcher.reset()
        self._ArcadeCamera__updateLodBiasForTanks()

    
    def _ArcadeCamera__changeZoomState(self, eScrollDirection):
        if eScrollDirection not in (EScrollDir.IN, EScrollDir.OUT):
            return None
        if None._ArcadeCamera__zoomStateSwitcher.isEmpty() or self._ArcadeCamera__bootcampCtrl.isInBootcamp():
            self._ArcadeCamera__updateProperties(state = None)
            return None
        state = None
        ...
        if eScrollDirection is EScrollDir.OUT:
            ...
            if not self._ArcadeCamera__updatedByKeyboard:
                ...
                self._ArcadeCamera__overScrollProtector.start(interval = interval, eScrollDirection = EScrollDir.OUT)
            if state:
                pass
            duration = _DEFAULT_ZOOM_DURATION
            if state:
                pass
    ...
    def _ArcadeCamera__hideCommanderCamHint(self):
        event_dispatcher.showCommanderCamHint(show = False)

    
    def _ArcadeCamera__updateCameraSettings(self, newDist):
        distMinMax = self._ArcadeCamera__distRange
        state = self._ArcadeCamera__zoomStateSwitcher.getCurrentState()
        if state:
            totalDiff = distMinMax.max - distMinMax.min
            if totalDiff is not 0:
                pass
            ratio = 0
    ...
    def _ArcadeCamera__compareCurrStateSettingsKey(self, key):
        state = self._ArcadeCamera__zoomStateSwitcher.getCurrentState()
        if state:
            return state.settingsKey == key

    
    def _ArcadeCamera__isSettingsEnabled(self, settingsKey):
    ...
    def _ArcadeCamera__cameraUpdate(self):
        ...
        if self._ArcadeCamera__shiftKeySensor is not None:
            self._ArcadeCamera__shiftKeySensor.update(1)
            if self._ArcadeCamera__shiftKeySensor.currentVelocity.lengthSquared > 0:
    ...
    def _ArcadeCamera__updateCollideAnimator(self, deltaTime):
    ...
    def _ArcadeCamera__calcFocalPoint(self, shotPoint, deltaTime):
        ...
        if absDiff < ArcadeCamera._FOCAL_POINT_MIN_DIFF or absDiff <= ArcadeCamera._FOCAL_POINT_CHANGE_SPEED * deltaTime:
            ...
            return shotPoint
        changeDir = (None - self._ArcadeCamera__focalPointDist) / absDiff
        self._ArcadeCamera__focalPointDist += changeDir * ArcadeCamera._FOCAL_POINT_CHANGE_SPEED * deltaTime
        return aimStartPoint + aimDir * self._ArcadeCamera__focalPointDist

    
    def _ArcadeCamera__calcAimOffset(self, oscillationsZoomMultiplier):
    ...
    def _ArcadeCamera__getAbsoluteDistRange(self):
        minDist = self._cfg['distRange'].min
        maxDist = self._cfg['distRange'].max
        for state in self._ArcadeCamera__zoomStateSwitcher:
            if not not (state.settingsKey):
                if self._ArcadeCamera__isSettingsEnabled(state.settingsKey):
    ...
    def _ArcadeCamera__calcCurOscillatorAcceleration(self, deltaTime):
        vehicle = BigWorld.player().getVehicleAttached()
        if vehicle is None:
            return Vector3(math_utils.VectorConstant.Vector3Zero)
        vehFilter = None.filter
    ...
    def _ArcadeCamera__updateOscillators(self, deltaTime):
        if not ArcadeCamera.isCameraDynamic():
            self._ArcadeCamera__impulseOscillator.reset()
            self._ArcadeCamera__movementOscillator.reset()
            self._ArcadeCamera__noiseOscillator.reset()
            return (Vector3(math_utils.VectorConstant.Vector3Zero), Vector3(math_utils.VectorConstant.Vector3Zero), 1)
        oscillatorAcceleration = None._ArcadeCamera__calcCurOscillatorAcceleration(deltaTime)
        self._ArcadeCamera__movementOscillator.externalForce += oscillatorAcceleration
        ...
        zoomMultiplier = math_utils.lerp(1, self._ArcadeCamera__dynamicCfg['zoomExposure'], relDist)
    ...
    def applyDistantImpulse(self, position, impulseValue, reason = ImpulseReason.ME_HIT):
        ...
        if distance < 1:
            distance = 1
        impulse.normalise()
        if reason == ImpulseReason.OTHER_SHOT and distance <= self._ArcadeCamera__dynamicCfg['maxShotImpulseDistance']:
            impulse *= impulseValue / distance
        elif reason == ImpulseReason.SPLASH or reason == ImpulseReason.HE_EXPLOSION:
            impulse *= impulseValue / distance
        elif reason == ImpulseReason.VEHICLE_EXPLOSION and distance <= self._ArcadeCamera__dynamicCfg['maxExplosionImpulseDistance']:
            impulse *= impulseValue / distance
        else:
            return None
        None.applyImpulse(position, impulse, reason)

    
    def _ArcadeCamera__applyNoiseImpulse(self, noiseMagnitude):
    ...
    def handleKeyEvent(self, isDown, key, mods, event = None):
        if self._ArcadeCamera__shiftKeySensor is None:
            return False
        if None.isKeyDown(Keys.KEY_CAPSLOCK) and mods & 4:
            if key == Keys.KEY_C:
                self.shiftCamPos()
            return self._ArcadeCamera__shiftKeySensor.handleKeyEvent(key, isDown)
        None._ArcadeCamera__shiftKeySensor.reset(Math.Vector3())
        return False
    ...
    def _readConfigs(self, dataSec):
        ...
        self._ArcadeCamera__movementOscillator = Math.PyCompoundOscillator(self._ArcadeCamera__movementOscillator, Math.PyOscillator(1, Vector3(50, 50, 50), Vector3(20, 20, 20), Vector3(0.01, 0, 0.01)))
        ...
        accelerationThreshold = readFloat(dynamicsSection, 'accelerationThreshold', 0, 1000, 0.1)
        self._ArcadeCamera__dynamicCfg['accelerationThreshold'] = accelerationThreshold
        self._ArcadeCamera__dynamicCfg['accelerationMax'] = readFloat(dynamicsSection, 'accelerationMax', 0, 1000, 0.1)
        self._ArcadeCamera__dynamicCfg['maxShotImpulseDistance'] = readFloat(dynamicsSection, 'maxShotImpulseDistance', 0, 1000, 10)
        self._ArcadeCamera__dynamicCfg['maxExplosionImpulseDistance'] = readFloat(dynamicsSection, 'maxExplosionImpulseDistance', 0, 1000, 10)
        self._ArcadeCamera__dynamicCfg['zoomExposure'] = readFloat(dynamicsSection, 'zoomExposure', 0, 1000, 0.25)
        ...
        maxAccelerationDuration = readFloat(dynamicsSection, 'maxAccelerationDuration', 0, 10000, ArcadeCamera._DEFAULT_MAX_ACCELERATION_DURATION)
        self._ArcadeCamera__accelerationSmoother = AccelerationSmoother(accelerationFilter, maxAccelerationDuration)
        self._ArcadeCamera__inputInertia = _InputInertia(self._ArcadeCamera__calculateInputInertiaMinMax(), 0)
        ...
        if advancedCollider is None:
            ...
            cfg['fovRatio'] = advancedCollider.readFloat('fovRatio', 2)
            cfg['rollbackSpeed'] = advancedCollider.readFloat('rollbackSpeed', 1)
            cfg['minimalCameraDistance'] = self._cfg['distRange'][0]
            cfg['speedThreshold'] = advancedCollider.readFloat('speedThreshold', 0.1)
            cfg['minimalVolume'] = advancedCollider.readFloat('minimalVolume', 200)
    ...
    def _readBaseCfg(self, dataSec):
        ...
        bcfg['fovMultMinMaxDist'] = MinMax(readFloat(dataSec, 'fovMultMinDist', 0.1, 100, 1), readFloat(dataSec, 'fovMultMaxDist', 0.1, 100, 1))
    ...
    def _readUserCfg(self):
        ...
        ucfg['keySensitivity'] = readFloat(dataSec, 'keySensitivity', 0, 10, 1)
        ucfg['sensitivity'] = readFloat(dataSec, 'sensitivity', 0, 10, 1)
        ucfg['scrollSensitivity'] = readFloat(dataSec, 'scrollSensitivity', 0, 10, 1)
